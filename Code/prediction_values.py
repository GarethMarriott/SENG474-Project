...
# Roxane Dewing -- V00853942
# Gathers nfl statistic data from the public api.fantasy.nfl
# Docs can be found at https://api.fantasy.nfl.com/v1/docs/service?serviceName=playersStats
#
#
#
...
def get_point_value(x):
    player = {x["name"] : x["weekPts"]}
    return player

def prosses_player_data(x):
    player = {}
    player["name"] = x["name"]
    player["position"] = x["position"]
    player["team"] = x["teamAbbr"]
    for stat in x["stats"]:
        for type in CONVERSION_TABLE:
            if str(type["id"]) == str(stat):
                player.update({type["name"] : x["stats"][stat]})

    return player

def main(ARGV):
    if len(ARGV) != 5:
        print("Usage: python project.py <Year> <Week> <path/to/outfile> <type_of_stat>")
        return 1
    # print(ARGV)

    url = 'https://api.fantasy.nfl.com/v1/players/stats?statType=weekStats&season='+ARGV[1]+'&week='+ARGV[2]+'&format=json'

    html = urlopen(url).read()

    d = json.loads(html)['players']

    d = list(filter(lambda x: str(x['position']) != 'DEF' , d))
    d = list(filter(lambda x: str(x['position']) != 'OL' , d))
    d = list(filter(lambda x: str(x['position']) != 'LB' , d))
    d = list(filter(lambda x: str(x['position']) != 'DB' , d))

    if ARGV[4] == "1":
        d = list(map(lambda x: get_point_value(x),d))
    else:
        d = list(map(lambda x: prosses_player_data(x),d))

    # print(d)
    print("Found data for " + str(len(d)) + " players")
